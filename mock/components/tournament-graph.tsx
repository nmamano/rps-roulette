...
import { outDegrees } from "@/lib/tournament"
  ...
  return (
    <div className="relative">
      <svg
        ...
        className="h-auto w-full max-w-[460px] touch-none select-none overflow-visible"
        role="img"
      ...
      >
        <defs>
          ...
          <marker id="arrow-win" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
            <path d="M0,0 L10,5 L0,10 z" className="fill-win" />
          </marker>
          <marker id="arrow-lose" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse">
            <path d="M0,0 L10,5 L0,10 z" className="fill-lose" />
          </marker>
          <marker id="arrow-decide" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="8" markerHeight="8" orient="auto-start-reverse">
            <path d="M0,0 L10,5 L0,10 z" className="fill-accent" />
          </marker>
          <filter id="soft" x="-40%" y="-40%" width="180%" height="180%">
            <feDropShadow dx="0" dy="4" stdDeviation="4" floodColor="oklch(0.6 0.08 25)" floodOpacity="0.18" />
          </filter>
        </defs>
        ...
        {points.map((_, i) =>
          points.map((_, j) => {
            ...
            const faded = (hover !== null && state === "idle" && !revealing) || (revealing && !isDeciding)
        ...
        {points.map((p, i) => {
          ...
          return (
            <g
              ...
              aria-label={interactive ? `Pick node ${labels[i]}, beats ${degrees[i]} of ${n - 1}` : undefined}
              onKeyDown={(e) => {
              ...
              style={{ transform: isHover || isYou || isOpp ? "scale(1.08)" : undefined, transformBox: "fill-box", transformOrigin: "center" }}
            >
              {isWinner && (
                <circle cx={p.x} cy={p.y} r={NODE_R + 9} className="fill-none stroke-accent" strokeWidth={4}>
                  <animate attributeName="r" values={`${NODE_R + 5};${NODE_R + 13};${NODE_R + 5}`} dur="1.2s" repeatCount="indefinite" />
                </circle>
              )}
              <circle
                cx={p.x}
